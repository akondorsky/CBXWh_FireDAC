...
try
 try
    ...
    if Length(Trim(E_Kol.Text))=  0 then
       begin
         Application.MessageBox('Не указано количество','Внимание',MB_ICONSTOP+MB_OK);
         ModalResult:=mrNone;
         Exit;
       end;
    if (Length(Trim(E_Days.Text))=  0) and (E_Days.Enabled) then
       begin
         Application.MessageBox('Не указано  количество дней ','Внимание',MB_ICONSTOP+MB_OK);
         ModalResult:=mrNone;
         Exit;
       end;
...
begin
  (Sender as Tform).Caption:= Dm.Qry_Parts.FieldByName('ZAYV').AsString +'. Добавление услуги.';
  E_Usl.Clear;
  E_Kol.Clear;
  E_Days.Clear;
