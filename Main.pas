...
begin
  ...
  Card.Connect1;
  psa:= Card.GetUID;
  SafeArrayAccessData(psa, ptr); //получаем указатель на данные
  SafeArrayGetLBound(psa,1,lb); //если всегда задавать LBound = 0, то можно не определять
...
begin
  ...
  try
    ...
    on E: Exception do
      begin
       Application.MessageBox(PWideChar(E.Message),'Ошибка соединения с базрй данных',MB_ICONERROR+MB_OK);
       Halt;
      end
  end;
  _DbName:=DB_Name;
...
if DM.Qry_Usl.FieldByName('USL_CHECK').AsInteger = 1 then
   begin
     Application.MessageBox('Услуга оплачена.Удаление невозможно','Внимание',MB_ICONSTOP+MB_OK);
     Exit;
   end;
if _User <>  DM.Qry_Usl.FieldByName('USERNAME').AsString then
   begin
     Application.MessageBox('Недостаточно прав для удаления','Внимание',MB_ICONSTOP+MB_OK);
     Exit;
   end;
